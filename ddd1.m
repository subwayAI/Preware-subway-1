function varargout = ddd1(varargin)
% DDD1 MATLAB code for ddd1.fig
%      DDD1, by itself, creates a new DDD1 or raises the existing
%      singleton*.
%
%      H = DDD1 returns the handle to a new DDD1 or the handle to
%      the existing singleton*.
%
%      DDD1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DDD1.M with the given input arguments.
%
%      DDD1('Property','Value',...) creates a new DDD1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ddd1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ddd1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ddd1

% Last Modified by GUIDE v2.5 28-Aug-2023 17:55:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ddd1_OpeningFcn, ...
                   'gui_OutputFcn',  @ddd1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ddd1 is made visible.
function ddd1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ddd1 (see VARARGIN)

% Choose default command line output for ddd1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ddd1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ddd1_OutputFcn(hObject, eventdata, handles) 
...
train_subway_label = [subway_label(1:ccc)];%;subway_label(60:95);subway_label(131:153)];
% 将第一类的31-59,第二类的96-130,第三类的154-178做为测试集
test_subway_eq = [subway_eq(ccc+1:end,:)];%;subway_eq(96:130,:);subway_eq(154:178,:)];
% 相应的测试集的标签也要分离出来
test_subway_label = [subway_label(ccc+1:end)];%;subway_label(96:130);subway_label(154:178)];
...
%% 将期望类别转换为向量
t_train=train_subway_label ;
t_test=test_subway_label;
t_train=ind2vec(t_train');
t_train_temp=train_subway_label %Train(:,4)';
...
%s1=[cc,data];
data=x;%(:,1:end-1)
subway_label=lei;%[ones(120,1);2*ones(120,1);3*ones(120,1)];
data=[data,subway_label];
%从1到2000间随机排序
k=rand(1,343);
[m,n]=sort(k);
...
test_subway_eq = dataset_scale( (mtrain+1):(mtrain+mtest),: );
%% SVM网络训练
model = svmtrain(train_subway_label, train_subway_eq, '-c 10.02 -g 10.1');

%% SVM网络预测
[predict_label, accuracy] = svmpredict(test_subway_label, test_subway_eq, model);
...
grid on;
cc=sum(test_subway_label-predict_label==0)/length(test_subway_label)
title(['train data svm ,acc:',num2str(cc)])

figure %创建混淆矩阵图
cm = confusionchart(test_subway_label,predict_label)
...
YTrain = train_subway_label';

test_x=test_subway_eq';
test_y=test_subway_label';
